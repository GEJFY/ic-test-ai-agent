...
def test_aws_xray_init_without_sdk():
    ...
    with patch.dict("os.environ", {}, clear=True):
        # aws_xray_sdk.core のインポートを失敗させる
...
def test_aws_xray_init_with_container_env():
    ...
    with patch.dict("os.environ", {"AWS_LAMBDA_FUNCTION_NAME": "test-function"}):
        ...
        mock_recorder = Mock()
        mock_xray_module.xray_recorder = mock_recorder
        mock_xray_module.patch_all = Mock()

        with patch.dict("sys.modules", {
            "aws_xray_sdk": Mock(),
            "aws_xray_sdk.core": mock_xray_module
        }):
            monitor = AWSXRay()
            assert monitor.enabled is True
            assert monitor.xray_recorder == mock_recorder
...
def test_track_metric_enabled():
    ...
    with patch.dict("os.environ", {"AWS_LAMBDA_FUNCTION_NAME": "test-func"}):
        ...
        with patch.dict("sys.modules", {
            ...
            assert mock_segment.put_metadata.called

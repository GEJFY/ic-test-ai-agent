...
- **対話型フィードバック**: AI評価結果にフィードバックして再評価を依頼可能
...
| A2 | 画像認識 | 承認印・署名・日付の抽出 + システム画面・フォーム分析（Vision API/OCR） |
...
証憑ファイルは以下の多段フィルタを経て処理されます：

```
VBA (全ファイルBase64化)
  ↓
Layer 0: 入力制御 (サイズ上限10MB/件、20件上限、合計50MB)
  ↓
Layer 3: Evidence Screening (統制・手続きとの関連性フィルタ)
  ↓
...
- 対話型フィードバック再評価機能（Excel G列にフィードバック入力→AI再評価）
- 判断のみ再評価（judgment_only）モードによる高速再評価対応
- 完全再評価（full）モードによる証憑再読み取り対応
- Excel列レイアウト変更（G:フィードバック, H:再評価回数, I-M+:出力）
...
**開発**: Goyo Systems | **更新日**: 2026-02-15

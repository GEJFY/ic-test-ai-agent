...
`main`/`develop`ブランチへのpush/PRで自動実行されます:
...
#### デプロイパイプライン（`deploy-{azure,aws,gcp}.yml`）

mainブランチへのpush（対象パス変更時）または手動実行（`workflow_dispatch`）で起動されます。

**トリガー条件（自動）**: `Dockerfile`, `src/**`, `platforms/local/**`, `infrastructure/{対象クラウド}/**` の変更

**手動実行パラメータ**: `environment`（staging / production）

```text
┌──────────────────────────────────────────────────────────────────┐
│  Deploy Pipeline (mainブランチ push + 手動実行)                    │
├──────────────────────────────────────────────────────────────────┤
│                                                                    │
│  [Test Job]                                                        │
│  └── pytest (e2e/integration除外)                                 │
│                                                                    │
│  [Security Job]                                                    │
│  └── scripts/audit_security.py                                    │
│                                                                    │
│  [Deploy Job] (Test + Security 成功後)                             │
│  ├── クラウド認証                                                  │
│  │   ├── Azure: az login (AZURE_CREDENTIALS)                      │
│  │   ├── AWS:   aws-actions/configure-aws-credentials              │
│  │   └── GCP:   google-github-actions/auth                        │
│  ├── Terraform init → plan → apply                                │
│  ├── Terraform outputからリソース名を自動取得                      │
│  │   ├── Azure: ACR名, Container App名, APIM情報                  │
│  │   ├── AWS:   ECR URL, App Runner ARN, API Gateway endpoint     │
│  │   └── GCP:   Artifact Registry URL, Cloud Run URL              │
│  ├── コンテナレジストリにログイン                                  │
│  ├── Dockerイメージ ビルド＆プッシュ (sha + latest タグ)           │
│  └── コンテナサービス更新                                          │
│      ├── Azure: az containerapp update                             │
│      ├── AWS:   aws apprunner start-deployment                     │
│      └── GCP:   gcloud run deploy                                  │
│                                                                    │
│  [Validate] (Deploy成功後)                                         │
│  └── scripts/validate_deployment.py --platform {azure,aws,gcp}    │
│                                                                    │
└──────────────────────────────────────────────────────────────────┘
```
...
| プラットフォーム | Secrets | 備考 |
| --------------- | ------- | ---- |
| Azure | `AZURE_CREDENTIALS`, `AZURE_RESOURCE_GROUP` | ACR認証はManaged Identity |
| AWS | `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` | ECR/App Runner権限が必要 |
| GCP | `GCP_SERVICE_ACCOUNT_KEY`, `GCP_PROJECT_ID` | Cloud Run Admin + AR Writer |

> 詳細な設定手順は [GitHub Actions CI/CD 設定ガイド](../../.github/workflows/README.md) を参照。

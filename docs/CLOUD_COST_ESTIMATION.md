...
# Part 5: PoC・本番環境 具体的見積もり

## 1. 前提条件

### 1.1 データ量定義

| フェーズ | 統制数 | サンプル数/統制 | 合計項目数 | 備考 |
|---------|-------|----------------|-----------|------|
| **PoC** | 35 | 100 | 3,500 | 初期検証フェーズ |
| **開発・テスト** | - | - | +1,000 | 開発中の試行錯誤 |
| **本番（想定）** | 35 | 300-400 | 10,500-14,000 | PoCの3-4倍 |

### 1.2 処理パラメータ

| パラメータ | 値 | 備考 |
|-----------|-----|------|
| 1項目あたり入力トークン | 5,500 | 計画+レビュー+タスク+判断 |
| 1項目あたり出力トークン | 2,100 | 全フェーズ合計 |
| 1項目あたりOCRページ数 | 2 | 平均値 |
| セルフリフレクション | 有効（MAX_REVISIONS=1） | 品質重視 |

---

## 2. PoC環境見積もり（3,500項目 + 開発1,000項目）

### 2.1 Azure

| リソース | 単価 | 使用量 | 月額（JPY） |
|---------|------|--------|------------|
| Azure Functions (Consumption) | 従量課金 | 4,500回 | ¥0（無料枠内） |
| Azure OpenAI (GPT-4o) 入力 | ¥375/1M tokens | 24.75M tokens | ¥9,281 |
| Azure OpenAI (GPT-4o) 出力 | ¥1,500/1M tokens | 9.45M tokens | ¥14,175 |
| Document Intelligence (Layout) | ¥1,500/1,000ページ | 9,000ページ | ¥13,500 |
| Storage (非同期モード) | 標準構成 | 2GB | ¥500 |
| Application Insights | ¥336/GB（5GB超過分） | 3GB | ¥0（無料枠内） |
| Log Analytics | ¥336/GB | 3GB | ¥0（無料枠内） |
| **PoC月額合計** | | | **¥37,456** |

**コスト削減オプション:**
- GPT-4o-mini使用時: LLMコスト約90%削減 → 約¥16,340/月
- セルフリフレクション無効化: LLMコスト約40%削減 → 約¥23,356/月

### 2.2 GCP

| リソース | 単価 | 使用量 | 月額（USD） | 月額（JPY@150） |
|---------|------|--------|-----------|----------------|
| Cloud Functions | 従量課金 | 4,500回 | $0 | ¥0 |
| Vertex AI (Gemini 1.5 Pro) 入力 | $1.25/1M tokens | 24.75M tokens | $30.94 | ¥4,641 |
| Vertex AI (Gemini 1.5 Pro) 出力 | $5.00/1M tokens | 9.45M tokens | $47.25 | ¥7,088 |
| Document AI (Layout Parser) | $10/1,000ページ | 9,000ページ | $90 | ¥13,500 |
| Cloud Storage | 標準 | 2GB | $0.05 | ¥8 |
| Firestore | 標準 | 最小 | $0.10 | ¥15 |
| **PoC月額合計** | | | **$168.34** | **¥25,252** |

### 2.3 AWS

| リソース | 単価 | 使用量 | 月額（USD） | 月額（JPY@150） |
|---------|------|--------|-----------|----------------|
| Lambda | 従量課金 | 4,500回 | $0 | ¥0 |
| Bedrock (Claude 3.5 Sonnet) 入力 | $3.00/1M tokens | 24.75M tokens | $74.25 | ¥11,138 |
| Bedrock (Claude 3.5 Sonnet) 出力 | $15.00/1M tokens | 9.45M tokens | $141.75 | ¥21,263 |
| Textract (Analyze Document) | $15/1,000ページ | 9,000ページ | $135 | ¥20,250 |
| S3 | 標準 | 2GB | $0.05 | ¥8 |
| DynamoDB | オンデマンド | 最小 | $0.50 | ¥75 |
| **PoC月額合計** | | | **$351.55** | **¥52,734** |

### 2.4 PoC見積もりサマリー

| プラットフォーム | 月額（JPY） | 3ヶ月PoC期間 | 備考 |
|----------------|------------|--------------|------|
| **Azure** | ¥37,456 | ¥112,368 | GPT-4o使用 |
| **GCP** | ¥25,252 | ¥75,756 | Gemini 1.5 Pro使用、最安 |
| **AWS** | ¥52,734 | ¥158,202 | Claude 3.5 Sonnet使用 |

---

## 3. 本番環境見積もり（10,500-14,000項目/月）

### 3.1 想定処理量（中央値: 12,000項目/月）

| パラメータ | 値 |
|-----------|-----|
| 月間処理項目数 | 12,000 |
| 入力トークン/月 | 66M tokens |
| 出力トークン/月 | 25.2M tokens |
| OCRページ数/月 | 24,000ページ |
...
|---------|-----|------------|
| Azure Functions | Premium EP1 | ¥23,000 |
| Azure OpenAI (GPT-4o) 入力 | 66M tokens | ¥24,750 |
| Azure OpenAI (GPT-4o) 出力 | 25.2M tokens | ¥37,800 |
| Document Intelligence | 24,000ページ | ¥36,000 |
| Storage (非同期) | 拡張構成 | ¥2,000 |
| Azure AD P1 | 5ユーザー | ¥4,500 |
| Application Insights | 12GB | ¥2,352 |
| Log Analytics | 18GB | ¥4,368 |
| Azure Monitor | アラート5ルール | ¥150 |
| **本番月額合計** | | **¥134,920** |
...
| リソース | 構成 | 月額（JPY@150） |
|---------|-----|----------------|
| Cloud Functions (2nd gen) | 標準 | ¥1,500 |
| Vertex AI (Gemini 1.5 Pro) 入力 | 66M tokens | ¥12,375 |
| Vertex AI (Gemini 1.5 Pro) 出力 | 25.2M tokens | ¥18,900 |
| Document AI | 24,000ページ | ¥36,000 |
| Cloud Storage | 10GB | ¥300 |
| Firestore | 拡張 | ¥750 |
| Cloud Monitoring | 標準 | ¥1,500 |
| **本番月額合計** | | **¥71,325** |
...
| リソース | 構成 | 月額（JPY@150） |
|---------|-----|----------------|
| Lambda | プロビジョニング | ¥3,000 |
| Bedrock (Claude 3.5 Sonnet) 入力 | 66M tokens | ¥29,700 |
| Bedrock (Claude 3.5 Sonnet) 出力 | 25.2M tokens | ¥56,700 |
| Textract | 24,000ページ | ¥54,000 |
| S3 | 10GB | ¥300 |
| DynamoDB | オンデマンド | ¥1,500 |
| CloudWatch | 標準 | ¥2,250 |
| **本番月額合計** | | **¥147,450** |

### 3.5 本番見積もりサマリー

| プラットフォーム | 月額（JPY） | 年額（JPY） | 備考 |
|----------------|------------|------------|------|
| **Azure** | ¥134,920 | ¥1,619,040 | Premium Functions含む |
| **GCP** | ¥71,325 | ¥855,900 | 最安、Gemini使用 |
| **AWS** | ¥147,450 | ¥1,769,400 | Claude使用、最高精度 |

---

## 4. 年間総コスト見積もり

### 4.1 PoC期間（3ヶ月）+ 本番運用（9ヶ月）

| プラットフォーム | PoC（3ヶ月） | 本番（9ヶ月） | 年間合計 |
|----------------|-------------|--------------|---------|
| **Azure** | ¥112,368 | ¥1,214,280 | **¥1,326,648** |
| **GCP** | ¥75,756 | ¥641,925 | **¥717,681** |
| **AWS** | ¥158,202 | ¥1,327,050 | **¥1,485,252** |

### 4.2 コスト最適化後の見積もり

| 最適化オプション | Azure | GCP | AWS |
|----------------|-------|-----|-----|
| 標準構成（上記） | ¥1,326,648 | ¥717,681 | ¥1,485,252 |
| 軽量モデル使用（-50%） | ¥850,000 | ¥500,000 | ¥950,000 |
| リフレクション無効化（-40%） | ¥900,000 | ¥550,000 | ¥1,050,000 |
| 両方適用（-70%） | ¥550,000 | ¥350,000 | ¥600,000 |
...
| セルフリフレクション | 有効（MAX_REVISIONS=1） |
| 年間予算 | 約¥1,300,000 |
| 選定理由 | Microsoft環境との親和性、GPT-4oの安定性 |
...
| 2.1 | 2026年2月 | PoC・本番環境具体的見積もり追加（35統制×100サンプル基準） |

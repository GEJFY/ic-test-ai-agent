...
| 列 | ヘッダー名 | 種別 | 説明 | 入力例 |
|----|-----------|------|------|--------|
| A | ID | 入力 | 項目の識別番号。一意の番号をつけてください | `001` |
| B | テスト対象 | 入力 | この行を処理対象にするか（TRUE/FALSE） | `TRUE` |
| C | カテゴリ | 入力 | 統制の分類名 | `統制環境` |
| D | 統制記述 | 入力 | 統制の内容を記述 | `経営者の誠実性と倫理観` |
| E | テスト手続き | 入力 | 評価の方法を記述 | `行動規範の文書確認` |
| F | 証憑リンク | 入力 | エビデンスファイルが格納されたフォルダパス | `C:\Evidence\001\` |
| G | フィードバック | 入力 | AI評価結果に対するフィードバック（再評価時に記入） | `署名日を確認して` |
| H | 再評価回数 | 自動 | フィードバック再評価の実行回数（自動更新） | `1` |
| I | 評価結果 | 出力 | 「有効」または「不備」 | `有効` |
...
| A   | B    | C          | D                    | E                    | F                | G              | H |
|-----|------|------------|----------------------|----------------------|------------------|----------------|---|
| ID  |対象  | カテゴリ    | 統制記述              | テスト手続き          | 証憑リンク        | フィードバック    | 回数|
| 001 | TRUE | 統制環境   | 経営者の誠実性と倫理観 | 行動規範の文書確認    | C:\Evidence\001\ |                | 0 |
| 002 | TRUE | リスク評価 | 不正リスクの評価       | リスク評価書のレビュー | C:\Evidence\002\ | 署名日を確認して | 1 |
| 003 | FALSE| 統制活動   | 職務分掌              | 権限一覧表の確認      | C:\Evidence\003\ |                | 0 |
```
...
| 出力列 | 内容 | 説明 |
|--------|------|------|
| I列（デフォルト） | 評価結果 | 「有効」または「不備」（setting.json の booleanDisplay で変更可） |
...
AI評価の結果に対してフィードバックを入力し、再評価を依頼できます。

#### フィードバックの入力方法

1. 初回評価を実行し、I列以降に結果が表示された状態にします
2. G列（フィードバック列）に修正指示を日本語で入力します
3. 「フィードバック再評価」マクロを実行します

#### フィードバックの入力例

| 状況 | フィードバック入力例 |
|------|---------------------|
| 署名確認が不足 | `証跡の署名日と承認者名を確認してください` |
| 判断根拠の修正 | `判断根拠に具体的な日付と金額を記載してください` |
| 再検討依頼 | `不備と判断されましたが、補足証跡を考慮して再評価してください` |

#### 再評価モード

フィードバック再評価マクロ実行時にモードを選択するダイアログが表示されます。

| モード | 説明 | 処理速度 | 用途 |
|--------|------|----------|------|
| **judgment_only**（デフォルト） | 判断・根拠のみ再生成 | 高速 | 根拠の表現修正、判断の再検討 |
| **full** | 証憑の再読み取りから完全再評価 | 通常 | 証跡の見落とし指摘、別観点での再評価 |

#### 再評価後の動作

- H列（再評価回数）が自動的にインクリメントされます
- I列以降の出力が再評価結果で上書きされます
- 複数回のフィードバック再評価が可能です（回数制限なし）
